...
| `clone`, `clone3` | Thread/process creation with flags. Denied because (a) no workload needed it (only `vfork` was observed — pip uses vfork/posix_spawn, thread count 0), and (b) `clone(CLONE_NEW*)` is a namespace-escape primitive. Verified: `threading.Thread().start()` → EPERM. **Known limitation**: CPython threading and thread-based multiprocessing are unavailable in v0.1. **Phase 10 remainder (2026-08-23)**: npm (Node) and cargo (Rust) dependency workflows GENUINELY and UNCONDITIONALLY require `clone3` — Node spawns a platform scheduler thread at startup for every workload (no flag avoids it), cargo spawns rustc children even for `cargo fetch` (real-filter measurement under the 70-syscall policy: node rc=139, cargo rc=101). DECISION: NO EXPANSION — clone3 is the S-014 single-process containment boundary; npm/cargo remain intentionally unsupported inside the sandbox and fail closed cleanly (policy.md §5 decision record). Node additionally needs `eventfd2`/`epoll_ctl`/`epoll_pwait`/`madvise`/`exit`, which also stay denied |
