...
| T-048 Malicious dependencies | S-033 | `test_content_attacks.py::DependencyAttackTests` (install payload executes inside, privileged fs modification attempt contained, no host modification, cleanup complete); **Phase 10 (v0.2 Step 4) now provides the INSTALL workflow itself** — `pip install --proxy http://10.255.254.0:8080` inside the real sandbox: destination validated by the proxy (disallowed source DENIED — `test_proxy.py::Phase10DependencyInstallTests`), direct networking blocked by the host firewall, AF_UNIX/NETLINK/PACKET denied by the BPF domain filter, clone/clone3/bind still denied, teardown clean (no veth/firewall/proxy leaks). The allowlist grew 69 → 70 (+fsync only, the single genuinely-required syscall; see policy.md §5). The v0.1 deferral by ADR-006 (network) is lifted for the pip path; Node/Rust workflows remain deferred | NATIVE VERIFIED (real boundary): dependency suite + Phase 10 install e2e (2026-08-23) — containment AND install workflow both verified |
