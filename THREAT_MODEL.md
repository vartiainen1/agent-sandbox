...
| T-048 Malicious dependencies | S-033 | `test_content_attacks.py::DependencyAttackTests` (install payload executes inside, privileged fs modification attempt contained, no host modification, cleanup complete) | NATIVE VERIFIED (real boundary): dependency suite — **CONTAINMENT only. The dependency-INSTALLATION workflow is DEFERRED in v0.1 (Phase 10): networked installs are prohibited by ADR-006 (network deferred to v0.2); even offline pip requires syscalls outside the 46-syscall policy incl. the forbidden socket/clone classes (native measurement 2026-08-22). Containment verified does NOT imply an install workflow exists.** |
