...
| Seccomp syscall allowlist derivation (Phase 1 pre-task) | **COMPLETE, container-validated** — 69-syscall HARDENED allowlist (tier0 29 + tier1 40; v0.2 Step 1 expanded the v0.1 46-syscall baseline by 23 network + native-toolchain syscalls, policy.md §5 change records), behaviorally verified |
...
| v0.2 Step 2 — allowlist network plumbing + socket domain filtering | **PLUMBING COMPLETE; PROXY/FIREWALL OUTSTANDING** — `network_mode` accepts `deny`/`allowlist`; veth-pair plumbing (create/move/configure/verify/cleanup) + supervisor↔PID-1 net-ready pipe wired; `socket` argument-filtered to AF_INET/AF_INET6 (AF_UNIX/AF_NETLINK/AF_PACKET EPERM — S-003/S-004 preserved); seccomp count unchanged (69). **NOT implemented yet (explicit, outstanding): the validating proxy and host-side iptables/nftables destination enforcement** — `network_mode="allowlist"` currently provides no outbound networking. Full unit 781 OK / 0 fail / 273 documented skips; veth root integration test verified on native VM (runbook: `RUNBOOK_v0.2_STEP2_NATIVE_VERIFICATION.md`) |
...
| Phase 20 — Release Hardening | **COMPLETE (implementation.md section 24)** — documented in `RELEASE_CHECKLIST.md`: every v0.1 security criterion has an evidence-backed status. v0.1 acceptance criteria satisfied (implementation.md section 26): runtime, filesystem/process/network isolation, privilege reduction, resource controls, environment sanitization, fail-closed init, session lifecycle, structured audit, CLI, regression tests, adversarial tests — all VERIFIED. Not claimed: production-ready, fully verified, signed release, independently reviewed. Release artifact reproducibility and signing: NOT YET VERIFIED (no sdist/wheel/signing configured). Independent Security Review: REQUIRED / NOT YET PERFORMED (implementation.md section 25). aarch64: SUBSTRATE-LIMITED / NOT VERIFIED. Full SECURITY_SPEC.md coverage: PARTIALLY VERIFIED only. Release tag: NOT CREATED (requires separate authorization). |
...
coreutils + CPython + git) makes exactly **69 syscalls** (tier0 29 +
tier1 40 — v0.2 Step 1 expanded the v0.1 46-syscall baseline by 23
network + native-toolchain syscalls; policy.md §5 change records); the
...
S-003/S-004 Unix-socket/credential isolation while leaving the v0.2
proxy path open (v0.2 Step 2).
...
- Known limitations: threads (`clone`/`clone3`) are denied by design;
  the x86_64 allowlist is exactly 69 (tier0=29, tier1=40 — v0.2 Step 1
  added the v0.2 networking syscalls + 15 native toolchain syscalls,
  policy.md §5 change records), and an independently derived aarch64
  allowlist (66 syscalls, commit a5338da lineage) is regression-gated
  with BPF construction verified — but aarch64 filter
  ...
  AF_INET/AF_INET6 (all other domains EPERM), and the allowlist network
  plumbing (veth pair) exists but the validating proxy and host-side
  iptables/nftables destination enforcement are NOT implemented yet —
  `network_mode="allowlist"` currently provides no outbound networking. The rootless
  ...
  workload  cannot bring its own loopback up); the derived 69-syscall
